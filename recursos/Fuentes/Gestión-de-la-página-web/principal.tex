...
\date{13 de febrero de 2026; revisado el 21 de febrero de 2026}
...
\begin{document}
    ...
    actualmente \LaTeX{} usa una carpeta \texttt{\_minted}. Para poder
    aprovechar la caché, deberé usar la opción \texttt{forzencache}, pero
    perece que no funciona correctamente, he abierto un \textit{issue} en
    GitHub para que los mantenedores del paquete lo revisen.
    
    Y parece ser
    que actualmente no es posible, por lo que he hecho un programita en Bash
    que copia los archivos de configuración que aparecen en el archivo inc.txt
    de cada proyecto de \LaTeX{} y lo copia en la carpeta \texttt{inc} del
    proyecto para que pueda compilarlo haciendo referencia a ese archivo
    copiado en inc. De llamar a este programita se encarga \texttt{make} cuando
    se llama \textbf{sin argumentos}.
    ...
    nuevos cambios. Y una vez comprobado que funciona bien, conectarme
    a la VPN que tengo de Proton (si no no se conecta a Internet) y hacer
