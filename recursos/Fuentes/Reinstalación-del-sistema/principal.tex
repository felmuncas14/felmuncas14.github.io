...
\hypersetup{
	pdfinfo={
		Subject={Reinstalación del sistema},
           	Creator={LaTeX},
           	CreationDate={D:20260124110000+00'00},
           	Producer={LaTeX}
    	}
}
...
\begin{document}
    ...
    \begin{enumerate}
	   ...
	   \item Synchting
		   ...
		   se bloquea.
	   \item Autenticador Proton
		   \subitem Exportación de las claves almacenadas.
	   \item Alacritty
		   \subitem \texttt{\$HOME/.config/alacritty/alacritty.toml}.
	   \item Rclone
		   ...
		   \texttt{/etc/\-systemd/system/\-rclone.service}. Debo tener la
		   ...
		   \texttt{/etc/syste\-md/\-system/rclone\_e.service}. Tengo que
	   ...
	   \item DWM
		   ...
		   \texttt{xinitr}, que se encuentra en la misma carpeta en la
    ...
    en \texttt{\~/.config/searxng/} a la configuración del contenedor. Para
    generar la
    ...
    Para hacer que se inicie desde el principio, se pueden seguir las
    instrucciones que haya actualizadas en la web y la página 5 del manual de la
    orden \texttt{podman-systemd.unit}. Ahora mismo lo que se deben utilizar son
    los \texttt{quadlet}. Actualmente lo he generado con \texttt{podlet}, que
    está todavía en beta, pero me servirá el ejemplo en caso de que hiciera
    falta actualizarlo.
    ...
    El \textit{quadlet} está en la carpeta
    \texttt{$\sim$/.config/containters/systemd/searxng.container}
    ...
    \section{Instalación del Autenticador de Proton}
    Simplemente sigo las indicaciones de instalación de su web. Y seguidamente
    importo el archivo de claves que he guardado previamente.
    ...
    la carpeta correspondiente a los servicios del sistema (quizá debería
    ponerlos junto a los servicios del usuario, pero no me voy a arriesgar a
    cambiarlo y que falle, por el momento).

    \section{Instalación de Qterminal}
    Debería venir instalado por defecto, no requiere una configuración más allá
    ...
    Seguir las instrucciones del archivo correspondiente.
    ...
    \texttt{AppImage} de su página oficial.

    \section{Instalación del tema de Thunar}
    ...
    sobre la instalación del mismo tema en GRUB. Me he quedao por aquí
    configurando.

    \section{Instalación del correo de Tuta}
    Simplemente instalo su aplicación e inicio sesión.

    \section{Instalación del BlueTooth}
    Instalar \texttt{blueman-manager}.
