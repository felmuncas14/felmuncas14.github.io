...
% For more information, please refer to <https://unlicense.org/>
    ...
    Los dos archivos que inician automáticamente este sistema y lo mantienen funcionando son: \texttt{~/.config/systemd/user.control/rclone.service} y \texttt{~/.config/systemd/user.control/rclone.service} con el siguiente contenido:
    
    \inputminted[breaklines, breakanywhere, breaksymbolleft="", breaksymbolright="", breakanywheresymbolpre="", breakanywheresymbolpost=""]{systemd}{inc/rclone.service}
    
    Y el archivo para la sincronización del escritorio:

    \inputminted[breaklines=true,breaksymbolleft=""]{systemd}{inc/rclone_e.service}
    ...
    Para actualizar la instancia, se puede ejecutar el comando \texttt{sudo rclone selfupdate}. Nótese que usa \texttt{go} para funcionar, luego no debo desinstalar nada de \texttt{go}. La configuración de \texttt{rclone} se encuentra en \texttt{\$HOME/.config/rclone/rclone.conf},
    que no se muestra porque es privado.
