...
\begin{document}
    ...
    También hay un problema con el ssh, pues no viene instalado por defecto. Para comprobar
    si es porque no esta disponible, basta poner en el \texttt{app.ini} de Forgejo que
    se encuentra en \texttt{com.termux/files/usr/bin/custom/conf} la opción
    «DISABLE\_SSH = true»,
    no es que se desactive en cualquier caso. Al estar activada esa opción se
    desactivan las opciones de ssh si resulta que no está bien instalado.

    No funciona, ni instalando y activando los agentes de SSH ni con el servidor
    de ssh nativo de Forgejo.
    ...
    Primero hay que ver cuál es la ip de mi móvil (en la configuración del teléfono),
    y actualizar mi archivo \texttt{/etc/hosts} para que el registro \texttt{movil}
    apunte realmente a la ip del móvil. Y actualizar el cortafuegos \texttt{ufw} para que
    permita las conexiones de podman al móvil. Para ver si ha habido un bloqueo puedo
    mirar con \texttt{sudo dmesg}. Elimino la regla antigua del cortafuegos con
    \texttt{sudo ufw status numbered}, apunto el número de la regla a eleminar. Y
    entonces ejecuto \texttt{sudo ufw delete <num\_regla>}. Finalmente hago:

    \texttt{sudo ufw route allow in on podman1 out on enx0e34bac885e7 to 10.47.157.26 port 3000}

    Poniendo la ip que tenga mi móvil en ese momento. Y sustituyendo
    \texttt{podman1} por el nombre de la interfaz desde la que se conecte
    Forgejo (aparecerá en \texttt{sudo dmesg}), y la interfaz de red de mi móvil
