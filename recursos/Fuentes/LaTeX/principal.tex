...
begin{minted}[breaklines, breakanywhere, brea    ksymbolleft="", breaksymbolright="", breakanywheresymbolpre="", breakanywheresymbolpost=""]{systemd}
   ...
   Para crear un proyecto en Vimtex, he de crear en una carpeta un archivo que se reconozca como principal. Este archivo, he configurado que sea \texttt{principal.tex}. La importancia, es que de todo el proyecto, este archivo es el único que se compila. De esta forma, si estoy trabajando con archivos aparte y compilo, solamente se compilará el \texttt{principal.tex}, que es el que incluirá o usará los otros archivos. El archivo de configuración donde se encuentra esto es: \texttt{~/.vim/pack/LaTeX/start/vimtex/plugin}, que a día de hoy, tiene el siguiente contenido añadido por mí:
...
let b:vimtex_main = "principal.tex"     " Esto establece
...
"
   ...
   Lo único que hago es establecer que el compilador (\texttt{latexmk}) compile en un directorio \texttt{build} por defecto, poner el visor de pdf como \texttt{evince} (el que uso desde hace mucho tiempo), y establecer el archivo principal en un proyecto como \texttt{principal.tex}.
   ...
   mismo directorio, los considerará Vimtex subordinados a ese. Nota: para que
   funcione  F4, que te lleva en el pdf a la línea que estás señalando en LaTeX,
   entonces tienes que crear obligatoriamente \texttt{principal.tex.latexmain}.
   Se podría hacer una modificación en
   \texttt{~/.vim/ftplugin/tex\_evinceSync.vim}, pero al actualizar el programa
   se revertiría mi cambio.
   ...
   \subsection{Trabajar en un archivo}
	   ...
	   El archivo de configuración que solamente se carga en los archivos \texttt{tex} se encuentra en \texttt{~/.vim/pack/LaTeX/start/vimtex/ftplugin/tex.vim}, que tiene el siguiente contenido añadido por mí. Nótese los útiles atajos para mostrar los números de línea en Vim.
...
let maplocalleader = " "        " Establece la tecla
" <localleader> a usar
let conceallevel = 2    " Esto es una recomendación del
" manual para la función de sustituir algunos comandos
" por texto.

set number
" Establece que el ancho de una línea debe ser 80, y rompe
" la línea en el primer espacio si no se cumple.
set textwidth=80
" Dibuja una columna en la línea 80 para ver cuál es el
...
" Atajo para llamar al que hace lo de synctex, si estoy
" escribiendo LaTeX y quiero ver en el pdf una zona de
" lo que estoy escribiendo, entonces pulso <F4> y me
" lleva en el visualizador de pdf a esa línea que
" corresponde al LaTeX en el que se encuentra el cursor.
nmap <F4> :call SVED_Sync()<CR>

" Atajo para desactivar la numeración de las líneas que
...
"
	   ...
	   De todos los atajos disponibles recomiendo los que siempre uso en todos los documentos.
\end{document}
